...
def ngme_sample(
    ...
    **model_kwargs,
    ...
    for i in range(max_length):
        ...
        # Get the unigrams and concat with new ids
        unigrams = torch.cat([input_ids.squeeze(0)[0, :].squeeze(0), next_tokens])
        input_ids = model.tokenizer.retokenize(unigrams, return_tensors="pt").to(input_ids.device)
